...
<head>
    ...
    <title><?php if (!empty ($userProfile['display_name'])) {echo htmlspecialchars($userProfile['display_name']);} else {echo htmlspecialchars($userProfile['firstname']).' '. htmlspecialchars($userProfile['lastname']);} echo ' (@'.htmlspecialchars($userProfile['username']).') / MetaRaid';?></title>
...
<body class="min-h-screen overflow-x-hidden dark:bg-black dark:text-white">
    ...
    <div class="relative flex items-center px-6">
        <div class="absolute -top-16 left-6">
            <img class="w-24 h-24 lg:w-36 lg:h-36 object-cover rounded-full border-4 border-white shadow-md dark:border-black transition-all duration-700 lg:mt-2 bg-white dark:bg-black" src="<?php if (!empty($userProfile['picture'])) { echo htmlspecialchars($userProfile['picture']);} else {echo '../Assets/pfdefault.png';}?>">
        </div>
    </div>
    ...
    <div class="mt-10 lg:mt-24 px-6">
        ...
        <small class="block text-[10px] md:text-sm lg:text-base xl:text-lg text-gray-600 mt-3 mb-3 dark:text-gray-400 transition-all duration-700">
            ...
            <i class="fa-solid fa-calendar"></i> A rejoint MetaRaid le <?php echo htmlspecialchars($userProfile['creation_date']); ?>
        </small>
